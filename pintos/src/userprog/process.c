...
setup_stack (void **esp, const char *file_name) 
{
  ...
  for(token = strtok_r(fn_copy, " ", &saveptr); token != NULL; token = strtok_r(NULL, " ", &saveptr)) {
    argc++;
  }
  ...
  for(token = strtok_r(fn_copy, " ", &saveptr), i = 0; token != NULL; token = strtok_r(NULL, " ", &saveptr), i++) {
    *esp -= strlen(token) + 1;
